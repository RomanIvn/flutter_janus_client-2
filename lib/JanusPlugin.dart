...
class JanusPlugin {
    ...
    if (webRTCHandle != null) {
      ...
      if (context.isUnifiedPlan) {
        webRTCHandle.localStream.getTracks().forEach((element) {
          webRTCHandle.peerConnection.addTrack(element, webRTCHandle.localStream);
        });
      } else {
  ...
  Future<bool> switchCamera() async {
    if (webRTCHandle.localStream != null) {
      final videoTrack = webRTCHandle.localStream.getVideoTracks().firstWhere((track) => track.kind == "video");
  ...
  Future<RTCSessionDescription> createOffer({bool offerToReceiveAudio = true, bool offerToReceiveVideo = true}) async {
    if (context.isUnifiedPlan) {
      await prepareTranscievers(true);
    }
    var offerOptions = {"offerToReceiveAudio": offerToReceiveAudio, "offerToReceiveVideo": offerToReceiveVideo};
  ...
  Future<RTCSessionDescription> createAnswer({bool offerToReceiveAudio = true, bool offerToReceiveVideo = true}) async {
    if (context.isUnifiedPlan) {
  ...
  Future prepareTranscievers(bool offer) async {
    ...
    if (transceivers != null && transceivers.length > 0) {
      transceivers.forEach((t) {
        if ((t.sender != null && t.sender.track != null && t.sender.track.kind == "audio") || (t.receiver != null && t.receiver.track != null && t.receiver.track.kind == "audio")) {
          if (audioTransceiver == null) {
            audioTransceiver = t;
          }
        }
        if ((t.sender != null && t.sender.track != null && t.sender.track.kind == "video") || (t.receiver != null && t.receiver.track != null && t.receiver.track.kind == "video")) {
    ...
    if (audioTransceiver != null && audioTransceiver.setDirection != null) {
      ...
      audioTransceiver = await webRTCHandle.peerConnection.addTransceiver(track: null, kind: RTCRtpMediaType.RTCRtpMediaTypeAudio, init: RTCRtpTransceiverInit(direction: offer ? TransceiverDirection.SendOnly : TransceiverDirection.RecvOnly, streams: []));
    }
    if (videoTransceiver != null && videoTransceiver.setDirection != null) {
      ...
      videoTransceiver = await webRTCHandle.peerConnection.addTransceiver(track: null, kind: RTCRtpMediaType.RTCRtpMediaTypeVideo, init: RTCRtpTransceiverInit(direction: offer ? TransceiverDirection.SendOnly : TransceiverDirection.RecvOnly, streams: []));
    }
  }
}
